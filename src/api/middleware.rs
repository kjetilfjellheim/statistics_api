 ...
 */
pub async fn timing_middleware(request: ServiceRequest, next: Next<impl MessageBody>) -> Result<ServiceResponse<impl MessageBody>, Error> {
 ...
 */
pub async fn digest_verification_middleware(mut request: ServiceRequest, next: Next<impl MessageBody>) -> Result<ServiceResponse<impl MessageBody>, Error> {
...
fn verify_digest(digest: &str, body: &[u8]) -> Result<(), ApplicationError> {
    let (algorithm, expected_digest) = digest.split_once('=').ok_or_else(|| ApplicationError::new(crate::model::apperror::ErrorType::Application, "Invalid digest format".to_string()))?;
